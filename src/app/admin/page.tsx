  ...
  const handleRefreshData = async () => {
    try {
      ...
      if (result.success) {
        ...
        setStatus("success");
        setMessage(
          `数据来源: ${result.source === "crawled" ? "爬取数据" : "模拟数据"}`
        );
      }
    } catch (error) {
      console.error(error)
            ...
            <CardContent className="space-y-4">
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <Badge variant="outline">{lastUpdate || "未知"}</Badge>
                </div>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {[
                ...
                >
                  <div
                    className={`w-8 h-8 rounded ${platform.color} flex items-center justify-center text-white text-sm`}
                  >
                    {platform.logo}
                  </div>
